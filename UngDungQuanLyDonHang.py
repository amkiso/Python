...
# Danh sách thống kê đầy đủ
    ...
    window = tk.Toplevel()
    window.title("Thống Kê Đơn Hàng - Xen Kẻ Màu Hàng")
    ...
    # Định nghĩa tag cho xen kẽ màu
    tree.tag_configure("Đã giao", background="#d4edda")        # xanh lá nhạt
    tree.tag_configure("Đang giao", background="#d1ecf1")      # xanh da trời nhạt
    tree.tag_configure("Đang vận chuyển", background="#fff3cd")# vàng nhạt
    tree.tag_configure("Đã nhập kho", background="#e2e3e5")    # xám nhạt
    tree.tag_configure("Đã hủy", background="#f8d7da")         # đỏ nhạt
    tree.tag_configure("default", background="#ffffff")        # mặc định
...
def Loc_San_Pham(parent_window, tree, order_manager):
    ...
    def apply_filters():
        ...
        for order in orders:
            ...
            # Lọc theo phương thức giao hàng
            if ptgh and match:
...
def MainProgram():
    ...
    def on_menu_select(option):
        if option == "Đăng xuất":
            Chose = messagebox.askyesno("Thông báo", f"Xác nhận đăng xuất ?")
            if not Chose:
                return
            else:
                Reset_giatri()
        ...
        if option == "Quản lý":
            ...
            if(current_user_role!="admin"):
                messagebox.showerror("Thông báo", "Chỉ admin mới có quyền xuất báo cáo")
                return
...
def QuenMatKhau():
    messagebox.showinfo("Thông báo", "Vui lòng liên hệ với admin để lấy lại mật khẩu")
def Login():
